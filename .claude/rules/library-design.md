...
hmntsk is a library that other applications embed. It must be **opinionated**: it does the right thing with sensible defaults and no configuration. The consumer must still keep **full control** to customise how it works in their application.
...
4. **Limits on flexibility are stated, never silently relaxed.** Where full flexibility would break a guarantee the library makes, offer the flexibility up to that line and document the line. Examples: sorting on an unindexed field breaks exact paging; retrying inside a sink breaks the relay's attempt budget. Do not accept the configuration and quietly degrade.
