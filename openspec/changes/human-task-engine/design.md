...
- Go baseline: **1.26 everywhere** — every `go.mod` and `go.work` declares it, and the Makefile pins `GOTOOLCHAIN` for every target rather than only for the linters, so a developer whose default toolchain has moved ahead builds and tests exactly what CI runs. An earlier arrangement pinned only `golangci-lint` and `govulncheck` (neither can parse a standard library newer than the one it was built against) and left `build` and `test` on whatever the developer happened to have, which is a difference that surfaces only in a pull request. CI needs no pin of its own: `actions/setup-go` installs the version and sets `GOTOOLCHAIN=local`, which the Makefile's `?=` defers to.
