...
- [x] 1.2 Red: add `"delivery/nats"` to the hardcoded module list in the root `docs_test.go`, and confirm the release-order test fails because the Makefile and `docs/releasing.md` do not name it.
- [x] 1.3 Green: name `delivery/nats` everywhere the workspace lists modules:
...
- [x] 2.1 Add `RunTestNATS(t, opts ...TestOption) *nats.Conn` to `delivery/nats/testutils.go` (design D10):
  ...
  - options for image, startup timeout and extra server arguments (needed for `--max_payload` in 4.5). _Implemented as `WithTestServerConfig` (a configuration file): nats-server 2.12.7 has no `--max_payload` flag and refuses to start when given one._
...
- [x] 2.2 Add a switchable TCP proxy test helper for outage cases, modelled on `delivery/redis`'s `brokerProxy`, which is in another module's test package and cannot be imported. Verify it through 4.4, whose closed-proxy case must fail if the proxy forwards while closed.

## 3. Construction and the shared contract

- [x] 3.1 Add inert stubs so tests compile, and verify `go build ./...`:
...
- [x] 3.2 Red: add a `TestNew` table covering both constructors.
...
- [x] 3.3 Green: validate in both constructors, with `ConfigurationError.Detail` saying how to fix each mistake. `TestNew` passes.

## 4. Plain-subject sink against a real server

- [x] 4.1 Red: add a `plainSuite` on one `RunTestNATS` server. A subscriber on `hmntsk.events.>` receives the message published by `NewSink`. Cases:
...
- [x] 4.2 Red: with no subscriber on the subject, `Deliver` returns `OutcomeDelivered`. Confirm it fails against the stub's unclassified outcome.
- [x] 4.3 Green: render subject, headers and body, then `PublishMsg` followed by `FlushWithContext` under the sink's timeout (D5). 4.1 and 4.2 pass.
- [x] 4.4 Red then green: outage cases through the proxy (2.2).
...
- [x] 4.5 Red then green: permanent failures. _`ErrHeadersNotSupported` is retryable, not permanent (design D8, changed with the user's approval): the client returns it before a connection's first connect. Pinned by `TestAConnectionThatNeverConnectedIsRetryable`._
...
- [x] 5.1 Red: add a `jetStreamSuite` whose `SetupSuite` asserts JetStream is available (2.1). Each case creates its own stream as the host would, capturing a unique prefix.
...
- [x] 5.2 Red: the same event delivered twice within the stream's duplicate window. Both attempts are delivered, and the stream holds one message.
- [x] 5.3 Red: with no stream capturing the subject:
...
- [x] 5.4 Red: with `WithExpectStream("expected")` and the subject captured by a different stream, `Deliver` is retryable and neither stream stores the message.
- [x] 5.5 Green: `PublishMsg` with `WithRetryAttempts(0)`, `WithMsgID(event.ID)` and the optional `WithExpectStream`; a duplicate acknowledgement counts as delivered; classification per D8 (D6, D7). 5.1–5.4 pass.
...
- [x] 5.6 In the same suite, the permanent cases from 4.5 hold for the JetStream sink: no identifier, and oversized against a small `--max_payload` server. Seen failing by temporarily classifying both as retryable.

## 6. Documentation

- [x] 6.1 Red: add `delivery/nats/docs_test.go`, modelled on `delivery/redis/docs_test.go`, scoped to a `# Publishing to NATS` section of `docs/delivery.md`. It asserts the section names:
...
- [x] 6.2 Green: write the section. It covers:
...
- [x] 6.3 Write the package doc and godoc stating each mode's guarantee where it is chosen: `NewSink` names the lost-without-subscriber case, `NewJetStreamSink` names the never-creates-streams rule. Point both to `docs/delivery.md`. Verify with `go doc -all .` in `delivery/nats` and `GOTOOLCHAIN=go1.26.8 make lint`.
