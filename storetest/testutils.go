...
// Pinned images. A moving tag would let a remote image update change what the
...
// consumer would want, and the reason is the adapter matrix: three drivers
...
// WithDatabase overrides the database name.
...
//
// The DSN asks for three things the engine needs and MySQL does not do by
// default:
//
//   - parsed times in UTC, because the engine stores UTC at microsecond
...
//   - multiStatements, so the published schema can be applied in one go.
...
// in the matrix — it makes the conformance suite cheap enough to run on every
...
// default and the engine's schema relies on them.
func RunTestSQLite(t *testing.T, _ ...TestOption) string {
	t.Helper()

	path := filepath.Join(t.TempDir(), "hmntsk.db")

	// Write-ahead logging so that a read on one connection is not blocked by an
	// open write transaction on another — the conformance suite has to be able
	// to look at committed state while a host-led transaction is still open.
	// Immediate transactions so that two writers queue on the busy timeout
	// rather than discovering the conflict at commit.
	return "file:" + path +
